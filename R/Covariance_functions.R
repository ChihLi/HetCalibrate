...
cov_gen <- function(X1, X2 = NULL, theta, type = c("Gaussian", "Matern5_2", "Matern3_2"), orthogonal = FALSE, ...){
  type <- match.arg(type)
  if(!orthogonal){
    if(type == "Gaussian")
    ...
    return(cov_ortho_gen(X1 = X1, X2 = X2, theta = theta, type = type, ...))
  }
...
## ' @export
partial_cov_gen <- function(X1, theta, type = "Gaussian", arg, ..., X2 = NULL, orthogonal = FALSE){
  if(!orthogonal){
    if(is.null(X2)){
      if(type == "Gaussian"){
        if(arg == "theta_k")
    ...
    return(partial_cov_ortho_gen(X1 = X1, X2 = X2, theta = theta, type = type, arg = arg, ...))
  }
