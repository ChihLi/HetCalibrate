...
logLikHom <- function(X0, Z0, Z, mult, theta, g, beta0 = NULL, covtype = "Gaussian", eps = sqrt(.Machine$double.eps), env = NULL,
                      cpara, orthogonal = FALSE, f.sim, df.sim = NULL, MC.num = 100, inputBounds = NULL){
  n <- nrow(X0)
  N <- length(Z)
  ...
  if(is.null(beta0)){
    ...
    # edit by chihli
    beta0 <- f.sim(X0, cpara)
  }

  if(orthogonal){
    C <- cov_gen(X1 = X0, theta = theta, type = covtype,
                 orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, cpara = cpara, MC.num = MC.num, inputBounds = inputBounds)
    if(!is.null(env)) env$C <- C
    Ki <- chol(C + diag(eps + g / mult))
    ldetKi <- - 2 * sum(log(diag(Ki))) # log determinant from Cholesky
    Ki <- chol2inv(Ki)
  ...
  #edit by chihli
  psi <- 1/N * ((crossprod(Z - rep(beta0, mult)) - crossprod((Z0 - beta0) * mult, Z0 - beta0))/g + psi_0)
...
dlogLikHom <- function(X0, Z0, Z, mult, theta, g, beta0 = NULL, covtype = "Gaussian",
                       eps = sqrt(.Machine$double.eps), components = c("theta", "g", "cpara"), env = NULL,
                       cpara, orthogonal = FALSE, f.sim, df.sim = NULL, MC.num = 100, inputBounds = NULL){
  k <- length(Z)
  n <- nrow(X0)
  ...
  if(!orthogonal){
    ...
    if("theta" %in% components){
      tmp1 <- rep(NA, length(theta))
      if(length(theta)==1){
        dC_dthetak <- partial_cov_gen(X1 = X0, theta = theta, type = covtype, arg = "theta_k",
                                      orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                      cpara = cpara, MC.num = MC.num, inputBounds = inputBounds)
        tmp1 <- k/2 * crossprod(KiZ0, dC_dthetak) %*% KiZ0 /((crossprod(Z) - crossprod(Z0 * mult, Z0))/g + psi) - 1/2 * trace_sym(Ki, dC_dthetak)
      }else{
        for(i in 1:length(theta)){
          dC_dthetak <- partial_cov_gen(X1 = X0[,i, drop = F], theta = theta[i], type = covtype, arg = "theta_k",
                                        orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                        cpara = cpara, MC.num = MC.num, inputBounds = inputBounds[,i,drop = F])
          tmp1[i] <- k/2 * crossprod(KiZ0, dC_dthetak) %*% KiZ0 /((crossprod(Z) - crossprod(Z0 * mult, Z0))/g + psi) - 1/2 * trace_sym(Ki, dC_dthetak)
        }
      }
    }
  ...
  if("cpara" %in% components){

    df_dcpara <- df.sim(X0, cpara)

    if(length(cpara) == 1){
      df_dcpara <- matrix(df_dcpara, ncol = 1)
    }

    if(orthogonal){
      tmp3 <- rep(0, length(cpara))
      dC_dcpara <- partial_cov_gen(X1 = X0, theta = theta, type = covtype, arg = "cpara",
                                   orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                   cpara = cpara, MC.num = MC.num, inputBounds = inputBounds)
      for(jj in 1:length(cpara)){
        tmp3[jj] <- k/2 * (2*crossprod(Z,rep(df_dcpara[,jj], mult))/g - 2 * crossprod(Z0 * mult, df_dcpara[,jj])/g
                           + 2 * crossprod(KiZ0, df_dcpara[,jj]) + crossprod(KiZ0, dC_dcpara[[jj]]) %*% KiZ0) / ((crossprod(Z) - crossprod(Z0 * mult, Z0))/g + psi) - 1/2 * trace_sym(Ki, dC_dcpara[[jj]])
      }

    }else{
      dC_dcpara <- matrix(0, ncol = n, nrow = n)
      tmp3 <- apply(df_dcpara, 2, function(df) {
        k/2 * (2*crossprod(Z,rep(df, mult))/g - 2 * crossprod(Z0 * mult, df)/g
               + 2 * crossprod(KiZ0, df) + crossprod(KiZ0, dC_dcpara) %*% KiZ0) / ((crossprod(Z) - crossprod(Z0 * mult, Z0))/g + psi) - 1/2 * trace_sym(Ki, dC_dcpara)
      })
    }

    # tmp3 <- k/2 * (2*crossprod(Z,rep(df_dcpara, mult))/g - 2 * crossprod(Z0 * mult, df_dcpara)/g
    #                + 2 * crossprod(KiZ0, df_dcpara) + crossprod(KiZ0, dC_dcpara) %*% KiZ0) / ((crossprod(Z) - crossprod(Z0 * mult, Z0))/g + psi) - 1/2 * trace_sym(Ki, dC_dcpara)
  }
...
##' @title Calibration of inexact computer models under homoskedastic noise based on maximum likelihood estimation.
...
##' @param cpara_max,cpara_min upper bound and lower bound of the calibration parameter(s).
...
##' @param inputBounds optional matrix of known boundaries in original input space, of size 2 times \code{ncol(X)}. This is only effective when \code{orthogonal=TRUE}. The default is \code{NULL} which uses the maximum and minimum values of \code{X}.
...
mleHomCalibrate <- function(X, Z, cpara_max = NULL, cpara_min = NULL, lower = NULL, upper = NULL, known = NULL,
                            ...
                            orthogonal = FALSE, f.sim, df.sim = NULL, MC.num = NULL, inputBounds = NULL){
  ...
  if(is.null(MC.num)) MC.num <- 30 * ncol(X0)
  ...
  if(is.null(cpara_min) || is.null(cpara_max)){
    cpara_min <- -100
    cpara_max <- 100
  }
  ...
  if(is.null(known$cpara) && is.null(init$cpara)) init$cpara <- (cpara_max + cpara_min)/2
  ...
  if(is.null(df.sim)){
    df.sim <- function(x, cpara){
      ...
      #   f.tmp <- function(para) c(f.sim(y, para))
      #   grad(f.tmp, cpara)
      # })
      f.tmp <- function(cpara.tmp, x.tmp) c(f.sim(x.tmp, cpara.tmp))
      gradient(f.tmp, cpara, x.tmp = x, centered = TRUE, pert = 1e-4)
    }
  }

  ## General definition of fn and gr
  fn <- function(par, X0, Z0, Z, mult, beta0, theta, g, env,
                 cpara, orthogonal, f.sim, df.sim, MC.num, inputBounds){
    ...
    if(is.null(cpara)){
      cpara <- par[idx:(idx - 1 + length(init$cpara))]
    }
    loglik <- logLikHom(X0 = X0, Z0 = Z0, Z = Z, mult = mult, theta = theta, g = g, beta0 = beta0, covtype = covtype, eps = eps, env = env,
                        cpara, orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds)
  ...
  ## Both known
  if(!is.null(known$g) && !is.null(known$cpara) && !is.null(known[["theta"]])){
    theta_out <- known[["theta"]]
    g_out <- known$g
    cpara_out <- known$cpara
    out <- list(value = logLikHom(X0 = X0, Z0 = Z0, Z = Z, mult = mult, theta = theta_out, g = g_out, beta0 = beta0, covtype = covtype, eps = eps,
                                  cpara = cpara_out, orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds),
    ...
    if(is.null(known$cpara)){
      parinit <- c(parinit, init$cpara)
      lowerOpt <- c(lowerOpt, cpara_min)
      upperOpt <- c(upperOpt, cpara_max)
    }
    ...
    out <- try(optim(par = parinit, fn = fn, gr = gr, method = "L-BFGS-B", lower = lowerOpt, upper = upperOpt, theta = known[["theta"]], g = known$g,
                     ...
                     cpara = known$cpara, orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds,
                     control = list(fnscale = -1, maxit = maxit, factr = 1e9), env = envtmp))
    ...
    if(is.null(known$g)) g_out <- out$par[length(init$theta)+1] else g_out <- known$g
    if(is.null(known[["theta"]])) theta_out <- out$par[1:length(init$theta)] else theta_out <- known[["theta"]]
    if(is.null(known$cpara)) cpara_out <- out$par[(length(init$theta)+2):length(out$par)] else cpara_out <- known$cpara
  ...
  if(is.null(beta0)){
    # beta0 <- drop(colSums(Ki) %*% Z0 / sum(Ki))
    # edit by chihli
    beta0 <- f.sim(X0, cpara_out)
  }

  if(orthogonal){
    Ki <- chol2inv(chol(add_diag(cov_gen(X1 = X0, theta = theta_out, type = covtype,
                                         orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, cpara = cpara_out, MC.num = MC.num, inputBounds = inputBounds),
                                 eps + g_out/ mult)))
  }
  ...
  #edit by chihli
  nu <- 1/N * ((crossprod(Z - rep(beta0, mult)) - crossprod((Z0 - beta0) * mult, Z0 - beta0))/g_out + psi_0)
  ...
  res <- list(theta = theta_out, g = g_out, nu_hat = as.numeric(nu), ll = out$value, nit_opt = out$counts,
              ...
              X0 = X0, Z0 = Z0, Z = Z, mult = mult, call = match.call(), cpara = cpara_out, # add cpara by chihli
              orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds, # add cpara by chihli
...
##' Predictions using a homoskedastic model object (of class \code{homCalibrate})
##' @param x matrix of designs locations to predict at (one point per row)
##' @param object an object of class \code{homCalibrate}; e.g., as returned by \code{\link[HetCalibrate]{mleHomCalibrate}}
...
##' \item \code{nugs}: nugget value at each prediction location, for consistency with \code{\link[HetCalibrate]{mleHomCalibrate}}.
##' }
##' @importFrom MASS ginv
##' @details The full predictive variance corresponds to the sum of \code{sd2} and \code{nugs}. See \code{\link[HetCalibrate]{mleHomCalibrate}} for examples. This code is modified from the source code of \code{\link[hetGP]{predict.homGP}}.
##' @method predict homCalibrate
##' @export
predict.homCalibrate <- function(object, x, xprime = NULL, ...){
  if(is.null(dim(x))){
    x <- matrix(x, nrow = 1)
  ...
  if(object$orthogonal){
    kx <- object$nu_hat * cov_gen(X1 = x, X2 = object$X0, theta = object$theta, type = object$covtype,
                                  orthogonal = TRUE, f.sim = object$f.sim, df.sim = object$df.sim, cpara = object$cpara, MC.num = object$MC.num, inputBounds = object$inputBounds)
  }else{
    kx <- object$nu_hat * cov_gen(X1 = x, X2 = object$X0, theta = object$theta, type = object$covtype)
  }
  ...
  mean <- as.vector(drop(object$f.sim(x, object$cpara) + kx %*% (object$Ki %*% (object$Z0 - object$beta0))))

  if(object$orthogonal){
    cor.x <- diag(cov_gen(x, x, theta = object$theta, type = object$covtype,
                          orthogonal = TRUE, f.sim = object$f.sim, df.sim = object$df.sim, cpara = object$cpara, MC.num = object$MC.num, inputBounds = object$inputBounds))
    if(object$trendtype == 'SK'){
      sd2 <- as.vector(object$nu_hat * cor.x - fast_diag(kx, tcrossprod(object$Ki, kx)))
    }else{
      sd2 <- as.vector(object$nu_hat * cor.x - fast_diag(kx, tcrossprod(object$Ki, kx)) + (1 - tcrossprod(rowSums(object$Ki), kx))^2/sum(object$Ki))
    }
  }else{
    if(object$trendtype == 'SK'){
...
logLikHet <- function(X0, Z0, Z, mult, Delta, theta, g, k_theta_g = NULL, theta_g = NULL, logN = FALSE, SiNK = FALSE,
  ...
  # edit by chihli
  psi <- 1/N * (crossprod((Z - rep(beta0, mult))/LambdaN, Z - rep(beta0, mult)) - crossprod((Z0 - beta0) * mult/Lambda, Z0 - beta0) + psi_0)
...
dlogLikHet <- function(X0, Z0, Z, mult, Delta, theta, g, k_theta_g = NULL, theta_g = NULL, beta0 = NULL, pX = NULL,
  ...
  if(is.null(components)){
    components <- c(list("theta", "Delta", "g", "cpara"))
  ...
  # edit by chihli
  psi <- drop((crossprod((Z - rep(beta0, mult))/LambdaN, Z - rep(beta0, mult)) - crossprod((Z0 - beta0) * mult/Lambda, Z0 - beta0)) + psi_0)
  ...
  if("theta" %in% components){
    ...
    for(i in 1:length(theta)){
      if(!orthogonal){
        ...
        if(length(theta) == 1){
          dC_dthetak <- partial_cov_gen(X1 = X0, theta = theta, arg = "theta_k", type = covtype,
                                        orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                        cpara = cpara, MC.num = MC.num, inputBounds = inputBounds) # partial derivative of C with respect to theta
        }else{
          dC_dthetak <- partial_cov_gen(X1 = X0[, i, drop = FALSE], theta = theta[i], arg = "theta_k", type = covtype,
                                        orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                        cpara = cpara, MC.num = MC.num, inputBounds = inputBounds[,i,drop = F]) # partial derivative of C with respect to theta
        }
      }
      ...
      if("k_theta_g" %in% components){
        ...
        dLogL_dtheta[i] <- N/2 * (crossprod((Z - rep(beta0, mult))/LambdaN * rep(dLdtk, times = mult), (Z - rep(beta0, mult))/LambdaN) - crossprod((Z0 - beta0)/Lambda * mult * dLdtk, (Z0 - beta0)/Lambda) +
  ...
  if(any(c("Delta", "g", "k_theta_g", "theta_g", "pX") %in% components)){
    ...
    # edit by chihli
    dLogLdLambda <- N/2 * ((fast_tUY2(mult, (Z - rep(beta0, mult))^2) - (Z0 - beta0)^2 * mult)/Lambda^2 + KiZ0^2/mult)/psi - (mult - 1)/(2*Lambda) - 1/(2*mult) * diag(Ki)
    if(logN){
  ...
  # Derivative of logL with respect to cpara
  if("cpara" %in% components){
    df_dcpara <- df.sim(X0, cpara)
    ...
    if(orthogonal){
      dLogL_dcpara <- rep(0, length(cpara))
      dC_dcpara <- partial_cov_gen(X1 = X0, theta = theta, type = covtype, arg = "cpara",
                                   orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                   cpara = cpara, MC.num = MC.num, inputBounds = inputBounds)
      for(jj in 1:length(cpara)){
        dLogL_dcpara[jj] <- N/2 * (2*crossprod((Z - rep(beta0, mult))/LambdaN, rep(df_dcpara[,jj], mult)) -
                                     2 * crossprod((Z0 - beta0) * mult/Lambda, df_dcpara[,jj]) +
                                     2 * crossprod(KiZ0, df_dcpara[,jj]) + crossprod(KiZ0, dC_dcpara[[jj]]) %*% KiZ0) / psi - 1/2 * trace_sym(Ki, dC_dcpara[[jj]])
      }

    }else{
      dC_dcpara <- matrix(0, ncol = n, nrow = n)
      dLogL_dcpara <- apply(df_dcpara, 2, function(df) {
        N/2 * (2*crossprod((Z - rep(beta0, mult))/LambdaN, rep(df, mult)) -
                 2 * crossprod((Z0 - beta0) * mult/Lambda, df) +
                 2 * crossprod(KiZ0, df) + crossprod(KiZ0, dC_dcpara) %*% KiZ0) / psi - 1/2 * trace_sym(Ki, dC_dcpara)
      })
    }
    # dLogL_dcpara <- N/2 * (2*crossprod((Z - rep(beta0, mult))/LambdaN, rep(df_dcpara, mult)) -
    #                          2 * crossprod((Z0 - beta0) * mult/Lambda, df_dcpara) +
    #                          2 * crossprod(KiZ0, df_dcpara) + crossprod(KiZ0, dC_dcpara) %*% KiZ0) / psi - 1/2 * trace_sym(Ki, dC_dcpara)
  }
...
##'
##'
##' @return a list which is given the S3 class \code{"hetCalbrate"}, with elements:
##' \itemize{
...
mleHetCalibrate <- function(X, Z, cpara_max = NULL, cpara_min = NULL, lower = NULL, upper = NULL,
  ...
  if(is.null(known$cpara)){
    components <- c(components, list("cpara"))
  }else{
    init$cpara <- known$cpara
  }
  ...
  if(is.null(init[["theta"]]) || is.null(init$Delta)){
    ...
    if(!is.null(known$g_H)){
      ...
      # chihli added
      if(any(mult > 2)){
        mean_var_replicates <- mean((fast_tUY2(mult, (Z - rep(Z0, times = mult))^2)/mult)[which(mult > 2)])
    ...
    modHom <- mleHomCalibrate(X = list(X0 = X0, Z0 = Z0, mult = mult), Z = Z, cpara_max = cpara_max, cpara_min = cpara_min, lower = lower,
                              known = list(theta = known[["theta"]], g = known$g_H, beta0 = known$beta0, cpara = known$cpara),
                              upper = upper, init = list(theta = init$theta, g = g_init, cpara = init$cpara), covtype = covtype, maxit = maxit,
                              noiseControl = list(g_bounds = c(noiseControl$g_min, noiseControl$g_max)), eps = eps, settings = list(return.Ki = rKI),
                              orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds)
    ...
    if(is.null(init$Delta)){
      predHom <- suppressWarnings(predict.homCalibrate(x = X0, object = modHom)$mean)
      ...
      ### chihli: if orthogonal = TRUE then modHom$nu_hat is not variance so it's not valid
      ...
      # chihli new
      if(logN){
        nugs_est0 <- log(drop(fast_tUY2(mult, nugs_est))/mult) # average
        nugs_est <- log(nugs_est)
      }else{
        nugs_est0 <- drop(fast_tUY2(mult, nugs_est))/mult # average
      }
  ...
  if((is.null(init$theta_g) && is.null(init$k_theta_g)) || is.null(init$g)){
    ...
    if(exists("nugs_est")){
      ...
      modNugs <- mleHomGP(X = X, Z = nugs_est,
                          lower = noiseControl$lowerTheta_g, upper = noiseControl$upperTheta_g,
                          init = list(theta = init$theta_g, g =  init$g), covtype = covtype, noiseControl = noiseControl,
                          maxit = maxit, eps = eps, settings = list(return.Ki = F))
  ...
  fn <- function(par, X0, Z0, Z, mult, Delta = NULL, theta = NULL, g = NULL, k_theta_g = NULL, theta_g = NULL, logN = FALSE, SiNK = FALSE,
                 ...
                 cpara = NULL, orthogonal, f.sim, df.sim, MC.num, inputBounds){
    ...
    loglik <- logLikHet(X0 = X0, Z0 = Z0, Z = Z, mult = mult, Delta = Delta, theta = theta, g = g, k_theta_g = k_theta_g, theta_g = theta_g,
                        ...
                        cpara = cpara, orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds)
  ...
  if(is.null(known$cpara)){
    ...
    lowerOpt <- c(lowerOpt, cpara_min)
    upperOpt <- c(upperOpt, cpara_max)
    if(trace > 2) cat("cpara: ", init$cpara, "\n")
  }
  ...
  if(!is.null(components)){
    ...
    if(!is.null(modHom)){
      ...
      modHom_tmp <- mleHomCalibrate(X = list(X0 = X0, Z0 = Z0, mult = mult), Z = Z,  cpara_max = cpara_max, cpara_min = cpara_min,
    ...
    if(is.null(known$cpara)){
      mle_par$cpara <- out$par[idx:(idx - 1 + length(init$cpara))]
      idx <- idx + length(init$cpara)
      if(trace > 1) cat("cpara |", mle_par$cpara, " | ", cpara_min, " | ", cpara_max, "\n")
    }
  ...
  if(penalty){
    ll_non_pen <- logLikHet(X0 = X0, Z0 = Z0, Z = Z, mult = mult, Delta = mle_par$Delta, theta = mle_par$theta, g = mle_par$g, k_theta_g = mle_par$k_theta_g, theta_g = mle_par$theta_g,
                            ...
                            cpara = mle_par$cpara, orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds)
  }else{
  ...
  if(!orthogonal){
    ...
    Ki <- chol2inv(chol(add_diag(cov_gen(X1 = X0, theta = mle_par$theta, type = covtype,
                                         orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim,
                                         cpara = mle_par$cpara, MC.num = MC.num, inputBounds = inputBounds),
                                 Lambda/mult + eps)))
  }
  ...
  if(is.null(known$beta0)){
    ...
    # edit by chihli
    mle_par$beta0 <- f.sim(X0, mle_par$cpara)
    #mle_par$beta0 <- f.sim(X0, mle_par$cpara) + drop(colSums(Ki) %*% (Z0-f.sim(X0, mle_par$cpara)) / sum(Ki))
  }

  psi_0 <- drop(crossprod(Z0 - mle_par$beta0, Ki) %*% (Z0 - mle_par$beta0))

  nu2 <- 1/length(Z) * (crossprod((Z - rep(mle_par$beta0, mult))/LambdaN, Z - rep(mle_par$beta0, mult)) - crossprod((Z0 - mle_par$beta0) * mult/Lambda, Z0 - mle_par$beta0) + psi_0)
  ...
  res <- list(theta = mle_par$theta, Delta = mle_par$Delta, nu_hat = as.numeric(nu2), beta0 = mle_par$beta0,
              ...
              nu_hat_var = nu_hat_var, call = match.call(), cpara = mle_par$cpara, # add cpara by chihli
              orthogonal = orthogonal, f.sim = f.sim, df.sim = df.sim, MC.num = MC.num, inputBounds = inputBounds, # add by chihli
...
##'Predictions using a heteroscedastic model object (of class \code{hetCalibrate})
##' @param x matrix of designs locations to predict at (one point per row)
##' @param object an object of class \code{hetCalibrate}; e.g., as returned by \code{\link[HetCalibrate]{mleHetCalibrate}}
...
##' See \code{\link[HetCalibrate]{mleHetCalibrate}} for examples. The code is modified from \code{\link[hetGP]{predict.hetGP}}.
##' @method predict hetCalibrate
##' @export
predict.hetCalibrate <- function(object, x, noise.var = FALSE, xprime = NULL, nugs.only = FALSE, ...){
  ...
  if(object$orthogonal){
    kx <- cov_gen(X1 = x, X2 = object$X0, theta = object$theta, type = object$covtype,
                  orthogonal = TRUE, f.sim = object$f.sim, df.sim = object$df.sim, cpara = object$cpara, MC.num = object$MC.num, object$inputBounds)
  }else{
    kx <- cov_gen(X1 = x, X2 = object$X0, theta = object$theta, type = object$covtype)
  }
  ...
  if(object$orthogonal){
    cor.x <- diag(cov_gen(x, x, theta = object$theta, type = object$covtype,
                          orthogonal = TRUE, f.sim = object$f.sim, df.sim = object$df.sim, cpara = object$cpara, MC.num = object$MC.num, inputBounds = object$inputBounds))
    if(object$trendtype == 'SK'){
      sd2 <- object$nu_hat * drop(cor.x - fast_diag(kx, tcrossprod(object$Ki, kx)))
    }else{
      sd2 <- object$nu_hat * drop(cor.x - fast_diag(kx, tcrossprod(object$Ki, kx)) + (1 - tcrossprod(rowSums(object$Ki), kx))^2/sum(object$Ki))
    }
  }else{
    if(object$trendtype == 'SK'){
  ...
  return(list(#mean = drop(object$beta0 + kx %*% (object$Ki %*% (object$Z0 - object$beta0))), #edit by chihli
    mean = drop(object$f.sim(x, object$cpara) + kx %*% (object$Ki %*% (object$Z0 - object$beta0))),
    sd2 = sd2,
    nugs = nugs,
    sd2var = sd2var,
    cov = cov))
